...
FROM crime 
WHERE year = 2016 -- Change from 2015 to 2016
LIMIT 10;
{sql connection = con}
```
-   No overall missing between primary and overall count 
...
```{sql connection = con}
SELECT primary_type, SUM(CASE WHEN arrest = TRUE THEN 1 ELSE 0 END) AS arrest_count
FROM crime 
WHERE year = 2016
...
```{sql connection = con}
SELECT EXTRACT(HOUR FROM date) as hour, SUM(CASE WHEN arrest = TRUE THEN 1 ELSE 0 END) AS arrest_count
FROM crime 
WHERE year = 2016
...
```{sql connection = con}
SELECT year, SUM(CASE WHEN arrest = TRUE THEN 1 ELSE 0 END) AS homicide_arrest_count
FROM crime 
...
```{sql connection = con}
SELECT year, district, SUM(CASE WHEN arrest = TRUE THEN 1 ELSE 0 END) AS arrest_count
FROM crime 
WHERE year = 2015 OR year = 2016
GROUP BY year, district
ORDER BY arrest_count DESC;
```
-   In 2015 and 2016, district 11 had the highest number of arrests.
...
```{r}
    df <- dbGetQuery(con, "SELECT primary_type, SUM(CASE WHEN arrest = TRUE THEN 1 ELSE 0 END) AS arrest_count FROM crime WHERE district = 11 AND year = 2016 GROUP BY primary_type ORDER BY arrest_count DESC;")
```
...
result1 <- crime_tbl %>%
  ...
  summarise(arrest_count = sum(if_else(arrest == TRUE, 1, 0))) %>%
